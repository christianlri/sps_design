CREATE OR REPLACE TABLE `{{ params.project_id }}.rl_dmart._srm_supplier_scorecard_supplier_score_overview_bo_rough` AS 
...
FROM  `fulfillment-dwh-production.rl_dmart._srm_supplier_scorecard_supplier_rough` as o
...
FROM
`fulfillment-dwh-production.rl_dmart._srm_supplier_efficiency_rough` as o
...
FROM
`fulfillment-dwh-production.rl_dmart._srm_supplier_availability_rough` 
...
FROM
`fulfillment-dwh-production.rl_dmart._srm_supplier_scorecard_operations_rough` as o
    ...
    CASE 
        ...
        WHEN b.time_granularity = 'Quarterly' THEN 
            LAG(b.Net_Sales_EUR, 4) OVER (
                PARTITION BY b.country_code, b.supplier_name, b.supplier_id, b.time_granularity, b.division_type, b.brand_owner_name
                ORDER BY 
...
-- ---------------------------------------------------- Metrics to calculate the Averages ----------------------------------------------------
...
b.brand_owner_name,
b.is_supplier_VMI,
CAST(NULL AS STRING) AS level_one,
