CREATE OR REPLACE TABLE `{{ params.project_id }}.rl_dmart._srm_supplier_score_overview_bo_categoriesl3` AS 
-- FORMER: _srm_supplier_scorecard_supplier_score_overview_bo_categories_l3
    WITH base AS (
    ...
    FROM `fulfillment-dwh-production.rl_dmart._srm_supplier_scorecard_supplier_categories_l3_rough`  as o
    ...
    efficient_movers,
    zero_movers,
    ROUND(gpv_eur,4) AS gpv_eur,
    perc_efficiency

    FROM
    `fulfillment-dwh-production.rl_dmart._srm_supplier_efficiency_category_l3_rough` as o
    ...
    FROM
    `fulfillment-dwh-production.rl_dmart._srm_supplier_availability_category_l3_rough` 
    ...
    FROM
    `fulfillment-dwh-production.rl_dmart._srm_supplier_scorecard_operations_categories_l3_rough` as o
        ...
        CASE 
            ...
            WHEN b.time_granularity = 'Quarterly' THEN 
                LAG(b.Net_Sales_EUR, 4) OVER (
                    PARTITION BY b.country_code, b.supplier_name, b.supplier_id, b.time_granularity, b.division_type, b.level_one, b.level_two, b.level_three, b.brand_owner_name
                    ORDER BY 
    ...
    AND
    b.level_two = s.level_two
    AND
    b.level_three = s.level_three
    ...
    AND
    b.level_two = a.level_two
    AND
    b.level_three = a.level_three
    ...
    AND
    b.level_two = e.level_two
    AND
    b.level_three = e.level_three
    ...
    CASE 
            WHEN b.global_entity_id IN ('TB_KW', 'TB_AE', 'TB_BH', 'HF_EG', 'TB_IQ', 'TB_JO', 'TB_OM', 'TB_QA', 'HS_SA') THEN
                CASE 
                    WHEN has_store_ops = 1 AND has_demand_planner = 1 THEN 'partial-VMI'
