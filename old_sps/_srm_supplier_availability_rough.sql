CREATE OR REPLACE TABLE `{{ params.project_id }}.rl_dmart._srm_supplier_availability_rough` AS
...
filtered_supplier AS (
  SELECT
    r.*
  FROM
    recent_supplier as r

  WHERE
    rn = 1
    AND warehouse_id IS NOT NULL
    AND r.global_entity_id IN ('TB_KW', 'TB_AE', 'TB_QA', 'PY_AR', 'FP_SG', 'PY_PE')
),
...
final AS (
  ...
  WHERE 
  a.period_day BETWEEN '2023-07-01' AND CURRENT_DATE()
  AND 
...
FROM
 ...
 LEFT JOIN
  `dh-darkstores-stg.dev_dmart._srm_supplier_supplier_hierarchy` as h
