CREATE OR REPLACE TABLE `{{ params.project_id }}.rl_dmart._srm_supplier_scorecard_operations_categories_l1_rough` AS

------ Making the products base first with the brand, brand owner and supplier data which can be retrieved through joining the sku_id and the global_entity_id between the products_base table and the supplier_performance_report_blended table ----------------------
...
WITH
 ...
 products_base AS (
    ...
    SELECT
    global_entity_id, sku_id,supplier_id, brand_owner_name, level_one
