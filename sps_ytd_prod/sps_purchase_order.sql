...
--   total_po_orders: count de todas las ordenes
--   total_compliant_po_orders: ordenes que pasan is_compliant_flag
...
CREATE OR REPLACE TABLE `{{ params.project_id }}.{{ params.dataset.cl }}.sps_purchase_order`
CLUSTER BY
   global_entity_id,
   time_period
AS
...
monthly_quarterly_data AS (
  ...
  FROM `{{ params.project_id }}.{{ params.dataset.cl }}.sps_purchase_order_month`
