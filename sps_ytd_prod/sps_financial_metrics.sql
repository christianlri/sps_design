...
monthly_quarterly_data AS (
  ...
  FROM (
    SELECT
      ...
      -- Las demas filas del mismo order+supplier reciben 0 -> no inflan el SUM
      CASE WHEN ROW_NUMBER() OVER (
        PARTITION BY src.global_entity_id, src.order_id, src.supplier_id, src.month
        ORDER BY src.sku_id  -- orden deterministico para reproducibilidad
      ) = 1
    ...
    FROM `{{ params.project_id }}.{{ params.dataset.cl }}.sps_financial_metrics_month` AS src
WHERE (
...
GROUP BY GROUPING SETS (
    -- ==========================================================
    -- FRONT-FACING CATEGORY DEEP-DIVE (YTD)
    -- ==========================================================
